...
class DataParser:
    ...
    def parse_text_for_location(self, article):
        ...
        possible_locations, counts = np.unique(filtered_locations, return_counts=True)
        
        try:
            # Find indices with most occurences
            indices = np.argwhere(counts == np.max(counts))
            # Out of these filter the longest matching name
            location = max(possible_locations[indices].tolist())[0]
...
if __name__ == "__main__":
    ...
    print("Article three word city, location: %s, county: %s." % (data_parser.parse_text_for_location(article9), data_parser.parse_text_for_county(article9)))
